<script>
	...
	import { themeState, themeStateAsText } from '$store/styleConfig';
	// Scripts
	...
	onMount(async () => {
		...
		$themeState = $config.theme == darkTheme ? true : false;
		$themeStateAsText = $config.theme;
	});
</script>
...
{#if asToggle}
	<input
		...
		on:click={() => {
			...
			$config.theme = $themeState == true ? darkTheme : lightTheme;
			$themeStateAsText = $config.theme;
...
{:else if asSelect}
	<select
		...
		bind:value={$themeStateAsText}
		on:change={() => {
			$themeState = !$themeState;
			$themeStateAsText = $themeState ? darkTheme : lightTheme;
			$config.theme = $themeStateAsText;
			styleCfg.save($config);
		}}
	>
